...
if __name__ == '__main__':
    ...
    for h in steps:
        ...
        for x, y in zip(res[i][0], res[i][0]):
        ...
        for x, y in zip(res2[i][0], res2[i][0]):
            print(f'x: {round(x, 5)}, y: {round(y, 5)}')
        print()

        ans.append({
            "h": h,
            "Shooting": {'x': res[i][0],  'y': res[i][0]},
            "FD":       {'x': res2[i][0], 'y': res2[i][0]}
        })
