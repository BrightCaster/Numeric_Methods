...
x = [2, 3]
h = 0.1
Exact function: y = (abs(x))**(3/2)
Cauchy problem: x*(x - 1)y'' + (1/2)*y' - (3/4)*y = 0
'''


def cauchyProblem(x, y, y_der):
    return (x*(x**2-1)*y_der + (x**2+1)*y)/(x**2) #4
#    return (3/2*y-y_der)/(2*x*(x-1))
#    return -4 * x * y_der - (4 * x ** 2 + 2) * y #6
    
def pureFunction(x): 
    return 1/x*(1+math.exp((x**2)/2)) #4
#    return (abs(x))**(1.5)
#    return (1 + x) * math.exp(-x ** 2) #6
...
if __name__ == '__main__':
    ...
    h = 0.1
    y0 = math.sqrt(2**3)
    y_der = 3/2*math.sqrt(2)
    """
    a = 1
    b = 2
    h = 0.1  # 4
    y0 = 1+math.exp(1/2)
    y_der = 2*math.exp(1/2)-1
